PRO VSYM, Nvert, STAR=star, SKELETON=skeleton, POLYGON=polygon, $
    FILL=fill, ROT=rot, THICK=thick, color=color
...
; check if you're plotting on a white background (PostScript
; file) or a black background (X window):
  color= (!d.name eq 'PS') ? 0 : 255
...
IF po THEN BEGIN
    usersym, x, y, fill=fi, thick=thick, color=color
ENDIF ELSE IF sk THEN BEGIN
    xx = fltarr(2*nv+1) & yy = xx
    xx[inv2] = x
    yy[inv2] = y 
    usersym, xx, yy, thick=thick, color=color
ENDIF ELSE IF st THEN BEGIN
    ...
    yy[inv2] = y
    yy[inv2_1] = inner_y    
    usersym, xx, yy, fill=fi, thick=thick, color=color
ENDIF
